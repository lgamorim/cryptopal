...
public interface ICryptocurrencyService
{
    ...
    Task<ServiceResult<CurrentPriceView>> GetCurrentPriceAsync(GetCurrentPriceQuery query, CancellationToken cancellationToken = default);

    /// <summary>Gets the latest price for tokens identified by contract address on a platform.</summary>
    Task<ServiceResult<TokenPriceView>> GetTokenPriceAsync(GetTokenPriceQuery query, CancellationToken cancellationToken = default);

    /// <summary>Gets historical price, market cap, and volume series for a coin.</summary>
    Task<ServiceResult<HistoricalMarketDataView>> GetHistoricalMarketDataAsync(GetHistoricalMarketDataQuery query, CancellationToken cancellationToken = default);

    /// <summary>Gets detailed metadata and market snapshots for a coin.</summary>
    Task<ServiceResult<CoinDataView>> GetCoinDataAsync(GetCoinDataQuery query, CancellationToken cancellationToken = default);

    /// <summary>Gets developer repository activity for a coin on a historical date.</summary>
    Task<ServiceResult<DeveloperDataView>> GetDeveloperDataAsync(GetDeveloperDataQuery query, CancellationToken cancellationToken = default);
}
